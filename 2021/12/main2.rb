...
class Puzzle
  ...
  def valid_path?(path)
    ...
    return false if nodes['start'].to_i > 1
    return false if nodes['end'].to_i > 1
    filtered_values = []
    nodes.each do |node, count|
      filtered_values << count if small?(node)
    end
    return false if filtered_values.select{|x| x > 2}.size > 0
    return false if filtered_values.select{|x| x == 2}.size > 1
    true
  end
